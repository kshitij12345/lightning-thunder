...
class TELinear(TransformerEngineBaseModule):
    ...
    def prepare_forward(self, inp):
        assert inp.is_cuda, "TransformerEngine needs CUDA."

        self.set_activation_dtype(inp)
        self.init_fp8_metadata(num_gemms=1)

        FP8GlobalStateManager.add_fp8_tensors_to_global_buffer(self.fp8_meta, fp8_weights=self._get_fp8_params())

        return inp.contiguous()

    def cast_weight_to_fp8(self, weight, is_grad_enabled):
...
def make_te_linear_meta(is_grad_enabled: bool = False):
    def _te_functional_linear_meta(
        a: TensorProxy, w: TensorProxy, w_fp8: TensorProxy, w_t_fp8: TensorProxy, bias: None | TensorProxy
...
def _create_fp8_linear_bound_symbol(
    ...
    te_linear = TELinear(w.shape[1], w.shape[0])
    linear_fn = partial(te_linear, is_grad_enabled=is_grad_enabled)

    global LINEAR_CALLS_COUNTER
    name = f"te_linear_{LINEAR_CALLS_COUNTER}"
    prep_inp_name = f"te_linear_prepare_input_{LINEAR_CALLS_COUNTER}"
    cast_weight_name = f"te_linear_cast_weight_{LINEAR_CALLS_COUNTER}"
    ...
    def bind_postprocess_prep_inp(bsym: BoundSymbol) -> None:
        # This dict is then used by trace.python_ctx() to resolve the
        # BoundSymbol to the actual function.
        bsym._call_ctx = {prep_inp_name: te_linear.prepare_forward}
        bsym._import_ctx = {IMPORT_CTX_TE_KEY: te}
        bsym._object_ctx = {FP8_RECIPE_KEY: fp8_recipe}

    prep_input_meta = lambda x: TensorProxy(like=x)
    sym = Symbol(
        name=prep_inp_name,
        meta=prep_input_meta,
        is_prim=True,
        executor=transformer_engine_ex,
        _bind_postprocess=bind_postprocess_prep_inp,
    )
    updated_a = prep_input_meta(a)
    prep_input_bsym = sym.bind(a, output=updated_a)

    # Now we need to append the BoundSymbol to the current trace.
    trace = get_tracectx()
    trace.scopes[-1].append(prep_input_bsym)
    for p in chain(prep_input_bsym.flat_proxy_outs, prep_input_bsym.flat_proxy_args):
        trace.names.add(p.name)

    def bind_postprocess_cast_weight(bsym: BoundSymbol) -> None:
        # This dict is then used by trace.python_ctx() to resolve the
        # BoundSymbol to the actual function.
        bsym._call_ctx = {cast_weight_name: partial(te_linear.cast_weight_to_fp8, is_grad_enabled=is_grad_enabled)}
        bsym._import_ctx = {IMPORT_CTX_TE_KEY: te}
        bsym._object_ctx = {FP8_RECIPE_KEY: fp8_recipe}

    cast_weight_meta = lambda x: (
        TensorProxy(like=x, dtype=dtypes.float8_e4m3fnuz),
        TensorProxy(like=x, dtype=dtypes.float8_e4m3fnuz),
    )
    sym = Symbol(
        name=cast_weight_name,
        meta=cast_weight_meta,
        is_prim=True,
        executor=transformer_engine_ex,
        _bind_postprocess=bind_postprocess_cast_weight,
    )
    w_fp8, w_t_fp8 = cast_weight_meta(w)
    cast_weight_bsym = sym.bind(w, output=(w_fp8, w_t_fp8))

    # Now we need to append the BoundSymbol to the current trace.
    trace = get_tracectx()
    trace.scopes[-1].append(cast_weight_bsym)
    for p in chain(cast_weight_bsym.flat_proxy_outs, cast_weight_bsym.flat_proxy_args):
        trace.names.add(p.name)

    def bind_postprocess(bsym: BoundSymbol) -> None:
        # This dict is then used by trace.python_ctx() to resolve the
        # BoundSymbol to the actual function.
        bsym._call_ctx = {name: linear_fn}
        bsym._import_ctx = {IMPORT_CTX_TE_KEY: te}
        bsym._object_ctx = {FP8_RECIPE_KEY: fp8_recipe}
    ...
    bsym = sym.bind(updated_a, w, w_fp8, w_t_fp8, b, output=meta_fn(updated_a, w, w_fp8, w_t_fp8, b))
